...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run diffusion policy in MuJoCo environment")
    parser.add_argument("--checkpoint", required=True, help="Path to trained .ckpt")
    parser.add_argument(
        "--model_xml",
        default=str(ROOT_DIR / "dp_mujoco" / "models" / "universal_robots_ur10e" / "scene_custom.xml"),
    ...
    parser.add_argument("--alpha_dq", type=float, default=0.2, help="EMA smoothing on joint velocity")
    parser.add_argument("--alpha_grip", type=float, default=1.0, help="EMA smoothing on gripper command")
    parser.add_argument("--ignore_action_orientation", action="store_true", help="Keep current orientation.")
    parser.add_argument("--viewer_fps", type=float, default=30.0, help="Viewer sync FPS cap")
    parser.add_argument(
        "--home_q",
        type=float,
        nargs=6,
    ...
    parser.add_argument("--verbose_plan", action="store_true", help="Print each new predicted plan")
    parser.add_argument("--save_traj", action="store_true", help="Record qpos/qvel/qacc for replay.")
    parser.add_argument(
        "--save_traj_path",
...
def main() -> None:
    ...
    builder = ObservationBuilder(cfg, device, env, image_height=obs_h, image_width=obs_w)
    ...
    print(
        f"Policy-driven MuJoCo run started | policy_hz={args.policy_hz:.1f} | pred_horizon={pred_horizon} | "
        f"exec_horizon={exec_horizon} | obs={obs_h}x{obs_w}"
    )
    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        while viewer.is_running():
            if env.data.time < prev_sim_time:
                ...
                builder.initialize_history()
                traj_exec.reset(
                    builder.get_latest_eef_pos(),
                    builder.get_latest_eef_quat(),
                    builder.get_latest_gripper_qpos(),
                    env.data.time,
                )
                servo.reset(env.data.qpos[:6], gripper_cmd=float(env.data.qpos[6] if env.data.qpos.shape[0] > 6 else -0.2))
                safety_hold = False
            ...
            if traj_exec.has_active_action() and not safety_hold:
                target_pos, target_rot, gripper_cmd, _alpha = traj_exec.get_target(env.data.time)
                ...
                env.apply_joint_command(current_result["q_target"], current_result["gripper_cmd"])
            else:
                env.apply_joint_command(servo.q_target, servo.smooth_gripper_cmd)
            ...
            if kinematic_status.get("status", "").lower() != "ok":
                print(
                    f"[SAFETY CHECK] status={kinematic_status.get('status')} | reason={kinematic_status.get('reason')} | "
                    f"metrics={kinematic_status.get('metrics')}"
                )
                safety_hold = True
                traj_exec.clear()
                servo.reset(env.data.qpos[:6], gripper_cmd=float(env.data.qpos[6] if env.data.qpos.shape[0] > 6 else -0.2))
                env.apply_joint_command(env.data.qpos[:6], env.data.qpos[6] if env.data.qpos.shape[0] > 6 else -0.2)
            ...
            if dynamic_status.get("status", "").lower() != "ok":
                print(
                    f"[SAFETY CHECK] status={dynamic_status.get('status')} | reason={dynamic_status.get('reason')} | "
