...
namespace
{
constexpr int kInitialRealtimeStatePolls = 200;
constexpr auto kInitialRealtimeStatePollPeriod = std::chrono::milliseconds(10);
}  // namespace
...
hardware_interface::CallbackReturn URPositionHardwareInterface::on_activate(const rclcpp_lifecycle::State& previous_state)
{
  ...
  bool got_valid_initial_state = false;
  for (int i = 0; i < kInitialRealtimeStatePolls; ++i) {
    pos_init = ur_driver_->rt_interface_->robot_state_->getQActual();
    if (pos_init.size() == 6) {
      got_valid_initial_state = true;
      break;
    }
    std::this_thread::sleep_for(kInitialRealtimeStatePollPeriod);
  }

  if (!got_valid_initial_state) {
    RCLCPP_ERROR(rclcpp::get_logger("URPositionHardwareInterface"),
                 "Timed out waiting for the first valid realtime joint state during activation");
    ur_driver_->halt();
    initialized_ = false;
    system_interface_initialized_ = 0.0;
  ...
  for (unsigned int idx = 0; idx < 6; ++idx) {
    urcl_joint_positions_.at(idx) = pos_init.at(idx);
...
hardware_interface::return_type URPositionHardwareInterface::perform_command_mode_switch(const std::vector<std::string>& start_interfaces, const std::vector<std::string>& stop_interfaces)
{
  ...
  if (!initialized_ && !start_modes_.empty()) {
    RCLCPP_ERROR(rclcpp::get_logger("URPositionHardwareInterface"),
                 "Rejecting command mode switch before initial realtime state synchronization");
    start_modes_.clear();
    stop_modes_.clear();
