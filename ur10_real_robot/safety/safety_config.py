...
@dataclass
class SafetyConfig:
    velocity_stop_threshold: float = 1.2         # UR10e: Vitesse max sûre en teleop ~1.2 rad/s 
    
    acceleration_stop_threshold: float = 8.0     # UR10e: Limite d'accélération en usage nomrmal
    acceleration_emergency_threshold: float = 12.0 # Hardware emergency limit (UR10e abs max is ~15-20)
    acceleration_filter_window: int = 5
    acceleration_emergency_consecutive: int = 3

    # Jacobian / condition thresholds
    cond_threshold_stop: float = 300.0           # Arrêt avant la singularité dure
    manip_threshold_stop: float = 0.005          # Minimum measure of manipulability

    # hysteresis / consecutive cycle counts
    # On réduit le hysteresis_stop pour réagir plus vite sur un vrai robot
    consecutive_stop_count: int = 3
