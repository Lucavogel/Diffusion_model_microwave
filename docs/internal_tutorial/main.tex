...
\hypersetup{
  ...
  citecolor=lirmmblue,
  urlcolor=lirmmblue,
  pdftitle={Practical Workflow for Testing Visuomotor Robot Learning Systems},
...
\begin{titlepage}
  ...
  \vspace{1.5cm}
  {\Huge\bfseries\color{lirmmblue} Practical Workflow for Testing Visuomotor Robot Learning Systems\par}
  \vspace{0.5cm}
  {\LARGE From MuJoCo Prototyping to Physical Deployment\par}
  \vspace{1.2cm}
...
\input{chapters/02_mujoco_setup}
\input{chapters/03_teleoperation_recording}
\input{chapters/04_dataset_interface}
\input{chapters/05_training_server}
\input{chapters/06_deployment}
\input{chapters/07_evaluation_iteration}

\appendix
\input{appendices/A_openhaptics_install}
